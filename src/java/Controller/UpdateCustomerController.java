...
public class UpdateCustomerController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (c2 == null) {
            ...
            int emailChanged = 0;
            String custID = c2.getUserID();
            String phoneNum = request.getParameter("profile-phonenumber");
            String email = request.getParameter("profile-email");
            String gender = request.getParameter("profile-gender");

            try {
                ArrayList<Customer> custArrayList = custDA.displayAllCustomerRecords();
                ArrayList<Admin> adminArrayList = adminDA.getUsers();

                if (phoneNum.equals(c2.getPhoneNum()) && email.equals(c2.getEmail())) {
                    success++;
                } 
                else if(!phoneNum.equals(c2.getPhoneNum()) && !email.equals(c2.getEmail())){
                    phonenumChanged++;
                    emailChanged++;
                }
                else if (phoneNum.equals(c2.getPhoneNum()) && !email.equals(c2.getEmail())) {
                ...
                if (countError == 0) {
                    Customer c = new Customer(custID, phoneNum, email, gender);
                    custDA.updateCustomerRecords(c);
